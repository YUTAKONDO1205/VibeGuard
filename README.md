...
The CLI prints both, e.g. `vibeguard 0.1.3 (engine 0.1.0)`. The tool version is read from `package.json` at runtime, so it always matches the published package. The engine version has stayed at `0.1.0` while the tool advanced to `0.1.3` because releases `0.1.1`–`0.1.3` (vsce metadata fix, OK-state UX, license) did not change what VibeGuard detects. It also still reads `0.1.0` through the in-progress context-window confidence work, whose bump is deliberately deferred so that `0.2.0` names one settled engine rather than several successive ones; until it lands, compare against the `paper-ses-v0.1.3` tag rather than this field.
