...
それと**重複しない**、セキュリティ目線の2軸を扱う。設計は「自リポの監査メモ」ではなく、
**第三者が再現でき一般化可能な知見を生む研究設計**（研究設問・新規性・他SAST横断比較・
形式的指標・妥当性への脅威・再利用可能アーティファクト）として書く。詳細は [SCOPE.md](SCOPE.md)。
...
### 研究設問（RQ）と新規性
- **RQ1 可用性**: ルール正規表現は未信頼入力でハングさせられるか（ReDoS）。他SASTでも起きるか。
- **RQ2 機密性**: 「コードを端末外に出さない」主張を反証可能な手続きで検証できるか。
- **RQ3 完全性**: 意味保存変換は検出率をどれだけ下げるか。回避耐性はツール設計に依存するか。
- **RQ4 自己機構**: context-confidence 補正・抑制は隠蔽に悪用されうるか。緩和の境界は。

新規性の核は「VibeGuard が良い/悪い」ではなく**パターンベースSASTの一般的性質**として一般化する点。
そのため P0 実験は **Bandit/Semgrep 等のベースラインを必ず横断併走**させる（[SCOPE.md](SCOPE.md) §0, §2）。
