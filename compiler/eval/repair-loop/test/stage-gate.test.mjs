...
test('gcc is one UNSUPPORTED_VENDOR line with a count, and the absent families one NOT_ATTEMPTED line', () => {
  const rows = [
    { kind: 'erasure', cc: 'gcc-13' }, { kind: 'erasure', cc: 'gcc-13' }, { kind: 'erasure', cc: 'clang-18' },
  ];
  const g = unsupportedVendorLine(rows);
  assert.match(g, /^gcc-13: UNSUPPORTED_VENDOR -- 2 tracked erasure cell/);
  assert.match(g, /cannot load into gcc/);
